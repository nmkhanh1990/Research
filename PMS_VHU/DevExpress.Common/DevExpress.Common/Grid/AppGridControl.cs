...
namespace DevExpress.Common.Grid
{
    public static class AppGridControl
    {
        ...
        public static bool chkKhoa(PMS.Entities.TList<PMS.Entities.TrangThaiGiangVien> LstTTGV,string strMaGiangVien)
        {
            bool bcheck = false;
            for (int i = 0; i < LstTTGV.Count; i++)
            {
                if (LstTTGV[i].MaGiangVien.ToString()== strMaGiangVien)
                {
                    bcheck = (bool) LstTTGV[i].TrangThai;
                    break;
                }
            }
            return bcheck;
