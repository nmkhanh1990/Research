...
namespace PMS.BLL
{
    public static class UserInfo
    {
        ...
        public static string QLNhomQuyen { get; set; }
        public static TaiKhoan User { get; set; }
        public static UserTypes LoginType { get; set; }

        //Thêm vào xem tài khoản nào được phép xem tất cả khoa
        public static string PQTheoKhoa { get; set; }

        public static bool DuocPhepChot { get; set; }

        public static string IdChucNangChot { get; set; }
        ...
        public static bool? IsLogin(string userName, string password)
        {
            ...
            if (objTaiKhoan != null)
            {
                if (string.Compare(objTaiKhoan.MatKhau, password, false) == 0
                    || string.Compare(objTaiKhoan.MatKhau, PMS.Common.Globals.EncodeMD5(userName, password), false) == 0)
                {
                    PQTheoKhoa = GetByTaiKhoanTheoKhoa(objTaiKhoan.MaTaiKhoan);
                    ...
                    if (objNhomQuyen != null)
                    {
                        ...
                        if (objNhomQuyen.QlNhomQuyen != null)
                            QLNhomQuyen = objNhomQuyen.QlNhomQuyen;
                        else
                            QLNhomQuyen = string.Empty;
                    }
                    ...
                    {
                        if (ex.Message.Contains("CoreUis_Log") && ex.Message.Contains("invalid"))
                        {
                            throw new Exception("DB CoreUis_Log không tồn tại.");
                        }
                        else if (ex.Message.Contains("CoreUis_Log") && ex.Message.Contains("full"))
                        {
                            throw new Exception("Không đủ dung lượng để lưu log cho CoreUis_Log.");
                        }
                        else
                        {
                            DevExpress.XtraEditors.XtraMessageBox.Show(ex.Message, "Có lỗi khi lưu nhật ký", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        }
                    }
                    User = objTaiKhoan;
                    return true;
        ...
        public static string GetByTaiKhoanTheoKhoa(int strMaTaiKhoan)
        {
            string strResult = string.Empty;
            DataTable tbl = new DataTable();
            IDataReader reader= DataServices.PhanQuyenTheoKhoa.GetByMaNhomKhoa(strMaTaiKhoan);
            tbl.Load(reader);
            foreach (DataRow item in tbl.Rows)
            {
                strResult = item["MaKhoa"].ToString();
                DuocPhepChot = Convert.ToBoolean(item["DuocPhepChot"] != null ? item["DuocPhepChot"] : false);
                IdChucNangChot = item["IdChucNang"].ToString();
            }
            return strResult;
        }
