...
namespace PMS.UI.Forms.HeThong
{
    public partial class frmTaiKhoan : XtraForm
    {
        ...
        private void frmTaiKhoan_Load(object sender, EventArgs e)
        {
            ...
            AppGridView.ShowField(gridViewTaiKhoan, new string[] { "MaNhomQuyen", "HoTen", "TenDangNhap", "MatKhau", "Xem", "TrangThai", "ResetPassWordGv", "PhanQuyen" , "UserUis","PhanQuyenTheoKhoa"},
                new string[] { "Nhóm quyền", "Họ tên", "Tên truy cập", "Mật khẩu", "Xem", "Khoá tài khoản", "Reset mật khẩu GV", "Phân quyền chức năng" , "Tài khoản UIS","Phân quyền theo khoa"}, new int[] { 200, 200, 150, 70, 60, 100, 120, 150 , 200,150 });
            AppGridView.ShowEditor(gridViewTaiKhoan, NewItemRowPosition.Top);
            AppGridView.AlignHeader(gridViewTaiKhoan, new string[] { "MaNhomQuyen", "HoTen", "TenDangNhap", "MatKhau", "Xem", "TrangThai", "ResetPassWordGv", "PhanQuyen", "UserUis", "PhanQuyenTheoKhoa" }, DevExpress.Utils.HorzAlignment.Center);
            ...
            AppGridView.RegisterControlField(gridViewTaiKhoan, "PhanQuyenTheoKhoa", repositoryItemButtonEditPhanQuyenTaiKhoan);
            ...
            foreach (DataRow obj in tkUis.Rows)
            {
                ...
                list.Add(new CheckedListBoxItem(PMS.Common.Globals.IsNull(obj["MaTaiKhoan"], (string)obj["MaTaiKhoan"]),
                                                PMS.Common.Globals.IsNull(obj["HoTen"],(string) obj["HoTen"]).ToString(),
        ...
        private void gridViewTaiKhoan_RowCellClick(object sender, RowCellClickEventArgs e)
        {
            ...
            if (view.ActiveFilter.IsEmpty)
            {
               ...
               else if (view.FocusedColumn.FieldName == "PhanQuyen")
                {
                    TaiKhoan tk = gridViewTaiKhoan.GetFocusedRow() as TaiKhoan;
                    if (tk != null)
                    {
                        using (frmPhanQuyenTaiKhoan frm = new frmPhanQuyenTaiKhoan(tk))
                        {
                            frm.ShowDialog();
                        }
                    }
               ...
               else if (view.FocusedColumn.FieldName == "PhanQuyenTheoKhoa")
                {
                    TaiKhoan obj = gridViewTaiKhoan.GetFocusedRow() as TaiKhoan;
                    if (obj != null)
                    {
                        using (frmPhanQuyenTaiKhoanTheoKhoa frm = new frmPhanQuyenTaiKhoanTheoKhoa(obj.MaTaiKhoan,int.Parse(obj.MaNhomQuyen.ToString())))
                        {
                            frm.ShowDialog();
                        }
                    }
