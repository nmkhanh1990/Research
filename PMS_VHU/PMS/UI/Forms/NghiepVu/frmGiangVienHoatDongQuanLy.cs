...
namespace PMS.UI.Forms.NghiepVu
{
    public partial class frmGiangVienHoatDongQuanLy : DevExpress.XtraEditors.XtraForm
    {
        ...
        //        btnQuyDoi.ItemShortcut = new DevExpress.XtraBars.BarShortcut((Keys.Control | Keys.S));
        //        btnXoa.ItemShortcut = new DevExpress.XtraBars.BarShortcut((Keys.Control | Keys.Delete));
        //    }
        //}
        #endregion
        ...
        {
            InitializeComponent();
            _maTruong = PMS.Common.Globals.GetMaTruong(_listCauHinhChung, "Mã trường");
        }
        ...
        private void frmGiangVienHoatDongQuanLy_Load(object sender, EventArgs e)
        {
            ...
            AppGridView.ShowField(gridView1, new string[] { "Id", "MaGiangVien", "MaHoatDongQuanLy", "NoiDungCongViec", "NgayThucHien", "SoPhut", "HeSoQuyDoi", "SoTiet", "GhiChu", "XacNhan" }
                    , new string[] { "Id", "Giảng viên", "Hoạt động quản lý", "Nội dung công việc", "Ngày thực hiện", "Tổng thời gian (Giờ)", "Hệ số quy đổi", "Số tiết quy đổi", "Ghi chú", "Xác nhận" }
            ...
            AppGridView.SummaryField(gridView1, "SoTiet", "{0:n2}", DevExpress.Data.SummaryItemType.Sum);
            ...
            #region HocKy
            /*
            ckCboHocKy.Properties.SelectAllItemCaption = "Tất cả";
            ckCboHocKy.Properties.TextEditStyle = TextEditStyles.Standard;
            ckCboHocKy.Properties.Items.Clear();
            ...
            ckCboHocKy.Properties.Items.AddRange(listHK.ToArray());
            ckCboHocKy.Properties.SeparatorChar = ';';
            ckCboHocKy.CheckAll();
            */
            #endregion
            ...
            cboKhoaDonVi.DataBindings.Add("EditValue", bsKhoaDonVi, "MaKhoa", true, DataSourceUpdateMode.Never);
            #endregion
        ...
        {
            int icheck = 0;
            string iID = gridView1.GetRowCellDisplayText(gridView1.FocusedRowHandle, "Id").ToString();
            AppGridView.DeleteSelectedRows(gridView1,ref icheck);
            if(icheck==1)
            {             
                GiangVienHoatDongQuanLy itemsGV = new GiangVienHoatDongQuanLy();
                itemsGV.Id = int.Parse(iID);               
                DataServices.GiangVienHoatDongQuanLy.Delete(itemsGV);
                LoadDataSource();
            }
        }
        ...
        private void btnLuu_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                decimal _soPhut;
                decimal _heSoQuyDoi;
                decimal _soTietQuyDoi;
                foreach (DataRow row in dt.Rows)
                {
                    _soPhut = decimal.Parse(PMS.Common.Globals.IsNull(row["SoPhut"], "decimal").ToString());
                    _heSoQuyDoi = decimal.Parse(PMS.Common.Globals.IsNull(row["HeSoQuyDoi"], "decimal").ToString());
                    if (_heSoQuyDoi <= 0) continue;
                    _soTietQuyDoi = Math.Round(_soPhut / _heSoQuyDoi, 2);
                    row["SoTiet"] = _soTietQuyDoi;
                }
                Luu();
            }
            catch (Exception ex)
                ...
                if (XtraMessageBox.Show("Bạn có muốn lưu các thay đổi không ?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        foreach (DataRow gvhdql in dt.Rows)
                        {
                            ...
                            {
                                if (gvhdql["MaGiangVien"].ToString() == "" || gvhdql["MaHoatDongQuanLy"].ToString() == "")
                                {
