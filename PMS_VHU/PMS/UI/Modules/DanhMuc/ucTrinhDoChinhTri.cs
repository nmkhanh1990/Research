...
{
    public partial class ucTrinhDoChinhTri : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public ucTrinhDoChinhTri()
        {
            InitializeComponent();
        }

        private void ucTrinhDoChinhTri_Load(object sender, EventArgs e)
        {
            ...
            AppGridView.ShowField(gridViewKyThi, new string[] { "MaQuanLy", "TenTrinhDoChinhTri", "GhiChu", "NgayCapNhat", "NguoiCapNhat" },
                new string[] { "Mã trình độ chính trị", "Tên trình độ chính trị", "Ghi chú", "Ngày cập nhật", "Người cập nhật" },
                new int[] { 180, 200, 200, 99, 99 });
        ...
        void InitData()
        {
            bindingSourceKyThi.DataSource = DataServices.TrinhDoChinhTri.GetAll();
        }
        ...
        private void btnLuu_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            TList<TrinhDoChinhTri> list = bindingSourceKyThi.DataSource as TList<TrinhDoChinhTri>;
            if (list != null)
            {
                if (XtraMessageBox.Show("Bạn có muốn lưu các thay đổi không ?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    try
                    {
                        if (list.IsValid)
                        {
                            ...
                            DataServices.TrinhDoChinhTri.Save(list);
        ...
        private void gridViewKyThi_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            if (col.FieldName == "MaQuanLy" || col.FieldName == "TenTrinhDoChinhTri" || col.FieldName == "GhiChu")
            {
        ...
        {
            TrinhDoChinhTri obj = e.Row as TrinhDoChinhTri;
            if (obj != null)
            {
        ...
        {
            TrinhDoChinhTri obj = target as TrinhDoChinhTri;
            if (obj != null)
            {
                if (((TList<TrinhDoChinhTri>)bindingSourceKyThi.DataSource).FindAll(p => p.MaQuanLy == obj.MaQuanLy).Count > 1)
                {
