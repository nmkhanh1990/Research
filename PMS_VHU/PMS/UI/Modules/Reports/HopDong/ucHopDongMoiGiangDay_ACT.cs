...
{
    public partial class ucHopDongMoiGiangDay_ACT : DevExpress.XtraEditors.XtraUserControl
    {
        #region Variable
        ...
        public ucHopDongMoiGiangDay_ACT()
        {
            InitializeComponent();
        ...
        private void ucHopDongMoiGiangDay_ACT_Load(object sender, EventArgs e)
        {
            #region InitGridview
        ...
        private void btnLocDuLieu_Click(object sender, EventArgs e)
        {
            ...
            if (cboKhoaDonVi.EditValue != null && cboMaGiangVien.EditValue != null && cboNamHoc.EditValue != null && cboHocKy.EditValue != null)
            {
                ...
                IDataReader reader = DataServices.GiangVien.HopDongMoiGiangDay("", cboKhoaDonVi.EditValue.ToString(), cboMaGiangVien.EditValue.ToString(), cboNamHoc.EditValue.ToString(), cboHocKy.EditValue.ToString()); //cboBacDaoTao.EditValue.ToString()
                tbl.Load(reader);
        ...
        private void btnIn_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            try
            {
                ...
                foreach (DataRow r in vListBaoCao.Rows)
                {
                    ...
                    foreach (DataRow v in vListBaoCao.Rows)
                    {
                        ...
                        {
                            tongCong += (decimal)v["ThanhTienGiangDay"];
                        }
                    }
            ...
            catch (Exception ex)
            {
                ...
                {
                    XtraMessageBox.Show(ex.Message + "\nVui lòng chọn môn muốn in.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    XtraMessageBox.Show(ex.Message);
...
vListBaoCao = Common.XuLyGiaoDien.LayDuLieuIn(gridViewHopDongMoiGiangVien, bindingSourceHopDongGiangDay);
            ...
            if (vListBaoCao != null && vListBaoCao.Rows.Count > 0)
            {
                using (frmBaoCao frm = new frmBaoCao())
                {
                    ...
                    //{
                        frm.InHopDongMoiGiangDay_ACT("", "", "", "", "", "", "", "", "", "", "", "", "", cboMaGiangVien.EditValue.ToString(), "", 0, "", 0
                        , PMS.Common.Globals._cauhinh.TenTruong, "", PMS.Common.Globals._cauhinh.DaiDienHopDongThinhGiang
                        , PMS.Common.Globals._cauhinh.ChucVuDaiDienHopDongThinhGiang, PMS.Common.Globals._cauhinh.ChucVuDaiDienHopDongThinhGiang02
                        , PMS.Common.Globals._cauhinh.DiaChiDaiDien, PMS.Common.Globals._cauhinh.DienThoaiDaiDien, PMS.Common.Globals._cauhinh.FaxDaiDien, _ngayIn, vListBaoCao);
                    //}
                    frm.ShowDialog();
                }
            }
        ...
        private void btnInThanhLy_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            Cursor.Current = Cursors.WaitCursor;
            gridViewHopDongMoiGiangVien.FocusedRowHandle = -1;
            bindingSourceHopDongGiangDay.EndEdit();
            ...
            try
            {
                ...
                foreach (DataRow r in vListBaoCao.Rows)
                {
                    ...
                    //}
                    //r["ThanhTienTongCong"] = tongCong;
                    r["ThanhTienTongCongBangChu"] = PMS.Common.Globals.DocTien((decimal)r["ThanhTienTongCong"]);
                }
            }
            catch (Exception ex)
            {
...
vListBaoCao = Common.XuLyGiaoDien.LayDuLieuIn(gridViewHopDongMoiGiangVien, bindingSourceHopDongGiangDay);
            ...
            if (vListBaoCao != null && vListBaoCao.Rows.Count > 0)
            {
                using (frmBaoCao frm = new frmBaoCao())
                {
                    //for (int i = 0; i < vListBaoCao.Rows.Count; i++)
                    //{
                    frm.InThanhLyHopDongMoiGiangDay_CTIM(_ngayIn, vListBaoCao);
                    //}
                    frm.ShowDialog();
                }
            }
