...
{
    public partial class ucHeSoHocKy : XtraUserControl
    {
        ...
        {
            #region Init GridView HeSoHocKy
            ...
            bindingSourceHeSoHocKy.DataSource = DataServices.HeSoHocKy.GetAll();
        ...
        {
            HeSoHocKy obj = bindingSourceHeSoHocKy.Current as HeSoHocKy;
            if (obj != null)
            {
                if (obj.IsNew)
        ...
        private void btnLuu_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            TList<HeSoHocKy> list = bindingSourceHeSoHocKy.DataSource as TList<HeSoHocKy>;
            if (list != null)
            {
                if (XtraMessageBox.Show("Bạn có muốn lưu các thay đổi không ?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    try
                    {
                        if (list.IsValid)
                        {
                            ...
                            DataServices.HeSoHocKy.Save(list);
        ...
        {
            HeSoHocKy obj = target as HeSoHocKy;
            if (obj != null)
            {
                if (((TList<HeSoHocKy>)bindingSourceHeSoHocKy.DataSource).FindAll(p => p.MaQuanLy == obj.MaQuanLy).Count > 1)
                {
        ...
        {
            HeSoHocKy obj = e.Row as HeSoHocKy;
            if (obj != null)
            {
