...
namespace PMS.UI.Modules.DanhMuc
{
    public partial class ucGiamTruDinhMuc : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        void InitGridCDGTVT()
        {
            ...
            AppGridView.ShowField(gridViewGiamTruDinhMuc, new string[] { "NoiDungGiamTru", "MucGiam", "DonVi", "GhiChu", "NgayCapNhat", "NguoiCapNhat", "ChiTiet" },
                                    new string[] { "Nội dung giảm trừ", "Mức giảm", "Đơn vị", "Ghi chú", "D", "U", "Chi tiết" },
                                    new int[] { 400, 150, 100, 200, 99, 99, 80 });
            ...
            AppGridView.RegisterControlField(gridViewGiamTruDinhMuc, "ChiTiet", repositoryItemButtonEditChiTiet);
            AppGridView.HideField(gridViewGiamTruDinhMuc, new string[] { "NgayCapNhat", "NguoiCapNhat" });
            AppGridView.FixedField(gridViewGiamTruDinhMuc, new string[] { "ChiTiet" }, FixedStyle.Right);
            if (_maTruong =="HBU")
            {
        ...
        private void gridViewGiamTruDinhMuc_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                TList<GiamTruDinhMuc> list = (TList<GiamTruDinhMuc>)bindingSourceGiamTruDinhMuc.DataSource;
                GiamTruDinhMuc g = list[gridViewGiamTruDinhMuc.FocusedRowHandle];
                TList<ChiTietGiamTruDinhMuc> listChiTiet = DataServices.ChiTietGiamTruDinhMuc.Find("MaGiamTruDinhMuc = " + g.MaQuanLy);
                ChiTietGiamTruDinhMuc ct;
                if (listChiTiet == null || listChiTiet.Count == 0)
                {
                    ct = new ChiTietGiamTruDinhMuc();
                    ct.MaGiamTruDinhMuc = g.MaQuanLy;
                }
                else
                {
                    ct = listChiTiet[0];
                }
                new Forms.FormCon.frmCauHinhChiTiet(ct).ShowDialog();
            }
        }

        private void repositoryItemButtonEditChiTiet_Click(object sender, EventArgs e)
        {
            TList<GiamTruDinhMuc> list = (TList<GiamTruDinhMuc>)bindingSourceGiamTruDinhMuc.DataSource;
            GiamTruDinhMuc g = list[gridViewGiamTruDinhMuc.FocusedRowHandle];
            TList<ChiTietGiamTruDinhMuc> listChiTiet = DataServices.ChiTietGiamTruDinhMuc.Find("MaGiamTruDinhMuc = " + g.MaQuanLy);
            ...
            {
                for(int i = 1; i <= 3; i++) //3 chi tiết: giảng dạy, NCKH, HĐQL
                {
                    ct = new ChiTietGiamTruDinhMuc();
                    ct.MaGiamTruDinhMuc = g.MaQuanLy;
                    listChiTiet.Add(ct);
            ...
            //    ct = listChiTiet[0];
            //}
            new Forms.FormCon.frmCauHinhChiTiet(listChiTiet).ShowDialog();
        }
    }
}
