...
{
    public partial class ucThongKeTongHopThanhToanThuLaoTapSu : DevExpress.XtraEditors.XtraUserControl
    {
        #region Variable
        TList<CauHinhChung> _cauHinhChung = DataServices.CauHinhChung.GetAll();
        int lanchot = 0;
        #endregion
        public ucThongKeTongHopThanhToanThuLaoTapSu()
        {
            InitializeComponent();
        ...
        void InitData()
        {
            ...
            if (cboNamHoc.EditValue != null && cboDonVi.EditValue != null)
            {
                if(cboLanChot.EditValue != null)
                {
                    ...
                    IDataReader reader = DataServices.KetQuaThanhToanThuLao.TongHopKetQuaThanhToanTapSu(cboNamHoc.EditValue.ToString(), cboHocKy.EditValue.ToString(), cboDonVi.EditValue.ToString(), cboLoaiGiangVien.EditValue.ToString(), int.Parse(cboLanChot.EditValue.ToString()));
                    dt.Load(reader);
                    bindingSourceThongKe.DataSource = dt;
                }
                else
                {
                    Common.XuLyGiaoDien.ThongBao("Chưa chốt dữ liệu cho năm học, học kỳ được chọn.", MessageBoxIcon.Stop, true);
                }
            }
