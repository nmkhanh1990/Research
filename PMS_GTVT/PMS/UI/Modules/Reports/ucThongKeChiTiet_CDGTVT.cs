...
namespace PMS.UI.Modules.Reports
{
    ...
    { 
        ...
        void InitData()
        {
            ...
            cboDonVi.DataBindings.Add("EditValue", bindingSourceDonVi, "MaBoMon", true, DataSourceUpdateMode.Never);
        }
                ...
                using (SaveFileDialog sf = new SaveFileDialog { Filter = "(*.xls)|*.xls" })
                {
                    if (sf.ShowDialog() == DialogResult.OK)
                        if (sf.FileName != "")
                        {
                            ...
                            gridControl1.ExportToXlsx(sf.FileName, options);
        ...
        private void bgv_RowCellStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs e)
        {
            if (_dt == null || e.RowHandle < 0 || e.RowHandle >= _dt.Rows.Count) return;
            
            if (_dt.Rows[e.RowHandle]["SoDongDu"].ToString() == "0")
            {
                e.Appearance.BackColor = Color.FromArgb(220, 255, 240);
            }
            else
            {
                e.Appearance.BackColor = Color.FromArgb(230, 220, 255);
            }
        }
    }
}
