	...
	public sealed class Utility
	{
        ...
        public static IDataReader ExecuteReader(TransactionManager transactionManager, DbCommand dbCommand)
        {
			...
			try
			{
                ...
                if ("Reader" == "NonQuery" || "Reader" == "Scalar") 
                SaveLogPMS(dbCommand, transactionManager.Database.ConnectionString);
			}
			catch (Exception /*ex*/)
			{
				//PSB 2006-05-26: should we use EntLib ExceptionPolicy.HandleException?
		...
		public static IDataReader ExecuteReader(Database database, DbCommand dbCommand)
		{
			...
			try
			{
                ...
                if ("Reader" == "NonQuery" || "Reader" == "Scalar") 
                SaveLogPMS(dbCommand, database.ConnectionString);
			}
			catch (Exception /*ex*/)
			{
				//PSB 2006-05-26: should we use EntLib ExceptionPolicy.HandleException?
        ...
        public static int ExecuteNonQuery(TransactionManager transactionManager, DbCommand dbCommand)
        {
			...
			try
			{
                ...
                if ("NonQuery" == "NonQuery" || "NonQuery" == "Scalar") 
                SaveLogPMS(dbCommand, transactionManager.Database.ConnectionString);
			}
			catch (Exception /*ex*/)
			{
        ...
        public static DataSet ExecuteDataSet(TransactionManager transactionManager, DbCommand dbCommand)
        {
			...
			try
			{
                ...
                if ("DataSet" == "NonQuery" || "DataSet" == "Scalar") 
                SaveLogPMS(dbCommand, transactionManager.Database.ConnectionString);
			}
			catch (Exception /*ex*/)
			{
		...
		public static object ExecuteScalar(TransactionManager transactionManager, DbCommand dbCommand)
		{
			...
			try
			{
                ...
                if ("Scalar" == "NonQuery" || "Scalar" == "Scalar") 
                SaveLogPMS(dbCommand, transactionManager.Database.ConnectionString);
			}
			catch (Exception /*ex*/)
			{
    ...
    /// <exception cref="System.Data.Common.DbException">The command could not be executed.</exception>
        ...
        static void SaveLogPMS(DbCommand dbCommand, string connectionString)
        {
            ...
            try
            {
                ...
                if (flag)
                    _connectionString = CipherLib.PSCExtensions.DecodeString(connectionString) + ";Max Pool Size=1000";
                else
                    _connectionString = DecodeString(connectionString);
            }
            catch
            {
            ...
            string strComm = dbCommand.CommandText;
            string value = "";

            for (int i = 0; i < dbCommand.Parameters.Count; i++)
            {
                value += "<Error " + dbCommand.Parameters[i].ParameterName + " = \"" + dbCommand.Parameters[i].Value + "\" />";
            }
            ...
            sqlComm.CommandText = "Insert into CoreUis_Log.dbo.psc_SystemLog (HostName,UserID,Command,Value,UpdateDate,UpdateStaff,IPAddress, DataBaseName) values " + "('"
                    + hostName + "','" + userID + "','" + strComm + "','" + value + "', getdate() ,'" + userID + "','" + ip + "','" + dbName + "')";
