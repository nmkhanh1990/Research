...
{	
	...
	{		
		...
		public static TList<DanhMucQuyMo> Fill(IDataReader reader, TList<DanhMucQuyMo> rows, int start, int pageLength)
		{
			...
			for (int i = 0; i < pageLength; i++)
			{
					...
					))
				{
					...
					c.TinhTrang = (reader[DanhMucQuyMoColumn.TinhTrang.ToString()] == DBNull.Value) ? null : (System.Boolean?)reader[(DanhMucQuyMoColumn.TinhTrang.ToString())];
		...
		public static void RefreshEntity(IDataReader reader, PMS.Entities.DanhMucQuyMo entity)
		{
			...
			entity.TinhTrang = (reader[DanhMucQuyMoColumn.TinhTrang.ToString()] == DBNull.Value) ? null : (System.Boolean?)reader[(DanhMucQuyMoColumn.TinhTrang.ToString())];
