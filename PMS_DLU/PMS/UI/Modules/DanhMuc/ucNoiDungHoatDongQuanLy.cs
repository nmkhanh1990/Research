...
namespace PMS.UI.Modules.DanhMuc
{
    public partial class ucNoiDungHoatDongQuanLy : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private void ucNoiDungHoatDongQuanLy_Load(object sender, EventArgs e)
        {
            ...
            AppGridView.RegisterControlField(gridViewNoiDung, "MaCongViec", repositoryItemGridLookUpEditHoatDong);
        ...
        private void barButtonItem2_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            TList<NoiDungCongViecQuanLy> list = bindingSourceNoiDung.DataSource as TList<NoiDungCongViecQuanLy>;
            if (list != null)
            {
                if (XtraMessageBox.Show("Bạn có muốn lưu các thay đổi không ?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    try
                    {
                        if (list.IsValid)
                        {
                            ...
                            DataServices.NoiDungCongViecQuanLy.Save(list);
