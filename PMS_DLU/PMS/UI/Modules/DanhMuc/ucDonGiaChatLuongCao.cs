...
namespace PMS.UI.Modules.DanhMuc
{
    public partial class ucDonGiaChatLuongCao : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private void ucDonGiaChatLuongCao_Load(object sender, EventArgs e)
        {
            ...
            AppGridView.ShowField(gridViewDonGiaClc, new string[] {"PhanLoaiGiangVien","NhomMonHoc","MaHocHam","MaHocVi","DonGia","GhiChu","NgayCapNhat","NguoiCapNhat" }
                        , new string[] { "Phân loại giảng viên", "Nhóm môn học","Học hàm", "Học vị", "Đơn giá", "Ghi chú", "Ngày cập nhật", "Người cập nhật" }
                        , new int[] { 200, 150,120, 120, 100, 130, 100, 100 });
            AppGridView.AlignHeader(gridViewDonGiaClc, new string[] { "PhanLoaiGiangVien", "NhomMonHoc","MaHocHam", "MaHocVi", "DonGia", "GhiChu", "NgayCapNhat", "NguoiCapNhat" }, DevExpress.Utils.HorzAlignment.Center);
            AppGridView.RegisterControlField(gridViewDonGiaClc, "MaHocVi", repositoryItemGridLookUpEditHocVi);
            AppGridView.RegisterControlField(gridViewDonGiaClc, "MaHocHam", repositoryItemGridLookUpEditHocHam);
        ...
        private bool RuleCheckDuplicate(object target, ValidationRuleArgs e)
        {
            ...
            if (obj != null)
            {
                ...
                if (_maTruong == "BUH")
                {
                    if (((TList<DonGiaChatLuongCao>)bindingSourceDonGiaClc.DataSource).FindAll(p => p.PhanLoaiGiangVien == obj.PhanLoaiGiangVien & p.NhomMonHoc == obj.NhomMonHoc & p.MaHocVi == obj.MaHocVi & p.MaHocHam==obj.MaHocHam).Count > 1)
                    {
