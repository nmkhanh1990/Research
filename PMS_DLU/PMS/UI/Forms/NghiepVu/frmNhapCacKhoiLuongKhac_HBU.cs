...
{
    public partial class frmNhapCacKhoiLuongKhac_HBU : DevExpress.XtraEditors.XtraForm
    {
        ...
        TList<LoaiKhoiLuong> _listLoaiKhoiLuong;
        TList<QuyDoiGioChuan> _listQuyDoiGioChuan;
        ...
        void InitData()
        {
            ...
            _listLoaiKhoiLuong = DataServices.LoaiKhoiLuong.GetAll();
            bindingSourceLoaiKhoiLuong.DataSource = _listLoaiKhoiLuong;
            ...
            {
                _listQuyDoiGioChuan = DataServices.QuyDoiGioChuan.GetByMaLoaiKhoiLuong(cboLoaiKhoiLuong.EditValue.ToString());
        ...
        public frmNhapCacKhoiLuongKhac_HBU()
        {
            InitializeComponent();
        ...
        void InitRemainingHBU()
        {
            ...
            AppGridView.ShowField(gridViewKhoiLuong, new string[] { "MaGiangVien", "Ho", "Ten", "MaLoaiCongViec", "MaLop", "MaNhom", "SoLuong", "HeSoNhan", "HeSoQuyDoi", "TietQuyDoi", "GhiChu", "NgayCapNhat", "NguoiCapNhat" }
                    , new string[] { "Mã giảng viên", "Họ", "Tên", "Mã loại công việc", "mã lớp", "Nhóm", "Số bài", "Số tiết", "Hệ số", "Số buổi", "Ghi chú", "Ngày cập nhật", "Người cập nhật" }
                    , new int[] { 90, 115, 55, 150, 130, 130, 100, 100, 90, 250, 99, 99, 99 });
            AppGridView.ShowEditor(gridViewKhoiLuong, DevExpress.XtraGrid.Views.Grid.NewItemRowPosition.Top);
            AppGridView.AlignHeader(gridViewKhoiLuong, new string[] { "MaGiangVien", "Ho", "Ten", "MaLoaiCongViec", "MaLop", "MaNhom", "SoLuong", "HeSoNhan", "HeSoQuyDoi", "TietQuyDoi", "GhiChu", "NgayCapNhat", "NguoiCapNhat" }, DevExpress.Utils.HorzAlignment.Center);
            ...
            AppGridView.HideField(gridViewKhoiLuong, new string[] { "NgayCapNhat", "NguoiCapNhat", "MaLop", "MaNhom", "TietQuyDoi" });
            AppGridView.ReadOnlyColumn(gridViewKhoiLuong, new string[] { "Ho", "Ten", "HeSoNhan", "HeSoQuyDoi" });
        }
                ...
                if (XtraMessageBox.Show("Bạn có muốn lưu các thay đổi không ?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        foreach (ViewKhoiLuongCacCongViecKhac cv in list)
                        {
                            ...
                            {
                                string dtNgayCapNhat = cv.NgayCapNhat == null ? DateTime.Now.ToShortDateString()+" "+DateTime.Now.ToLongTimeString(): cv.NgayCapNhat;
                                string sNguoiCapNhat = cv.NguoiCapNhat == null ? UserInfo.UserName : cv.NguoiCapNhat;
                                string dTietQuyDoi = cv.TietQuyDoi == null ? "0" : cv.TietQuyDoi.ToString();
                                xmlData += "<Input M=\"" + cv.MaGiangVien
                                        + "\" S=\"" + cv.SoLuong
                                        + "\" ML=\"" + cv.MaLop
                                        + "\" MN=\"" + cv.MaNhom//PMS.Common.Globals.IsNull(cv.MaNhom, "string")
                                        + "\" G=\"" + cv.GhiChu
                                        + "\" D=\"" + dtNgayCapNhat 
                                        + "\" U=\"" + sNguoiCapNhat 
                                        + "\" L=\"" + cv.MaLoaiCongViec
        ...
        private void gridViewKhoiLuong_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            try
            {
                ...
                //}
                else if (col.FieldName == "MaLoaiCongViec")
                {
                    QuyDoiGioChuan objQDGC = _listQuyDoiGioChuan.Find(p => p.MaQuyDoi == int.Parse(e.Value.ToString()));
                    gridViewKhoiLuong.SetRowCellValue(e.RowHandle, "HeSoNhan", objQDGC.SoLuong);
                    gridViewKhoiLuong.SetRowCellValue(e.RowHandle, "HeSoQuyDoi", objQDGC.HeSo);
                }
                Cursor.Current = Cursors.Default;
            }
            catch (Exception ex)
