...
namespace DevExpress.Common.Grid
{
    public static class AppGridView
    {
        ...
        public static void DeleteSelectedRows(GridView grid,ref int i)
        {
            if (grid.RowCount > 0)
                if (grid.GetSelectedRows().Length > 0)
                    if (XtraMessageBox.Show(String.Format("Bạn có muốn xóa {0} dòng đã chọn không ?", grid.GetSelectedRows().Length), "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
        ...
        public static void NextPrev(GridView gdv, int iNext, int iflag, ref int iMoveNext)
        {
            gdv.CloseEditor();
            for (int i = 0; iflag > iMoveNext ? i < iflag - iMoveNext : i < iMoveNext - iflag; i++)
            {
                if (iNext <= iflag && iNext > iMoveNext)
                    gdv.MoveNext();
                else if (iNext < iMoveNext)
                    gdv.MovePrev();
            }
            if (iflag == 0 && iMoveNext == 0)
                gdv.MoveNext();
            gdv.ShowEditor();
        }
