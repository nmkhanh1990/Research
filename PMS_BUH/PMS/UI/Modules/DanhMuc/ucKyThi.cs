...
{
    public partial class ucKyThi : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private void ucKyThi_Load(object sender, EventArgs e)
        {
            ...
            AppGridView.ShowField(gridViewKyThi, new string[] { "MaKyThi", "TenKyThi", "GhiChu", "NgayCapNhat", "NguoiCapNhat" },
                new string[] { "Mã kỳ thi", "Tên kỳ thi", "Ghi chú", "Ngày cập nhật", "Người cập nhật" },
                new int[] { 90, 200, 200, 99, 99 });
        ...
        void InitData()
        {
            bindingSourceKyThi.DataSource = DataServices.KyThi.GetAll();
        }
        ...
        private void btnLuu_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            TList<KyThi> list = bindingSourceKyThi.DataSource as TList<KyThi>;
            if (list != null)
            {
        ...
        private void gridViewKyThi_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            if (col.FieldName == "MaKyThi" || col.FieldName == "TenKyThi" || col.FieldName == "GhiChu")
            {
        ...
        private void gridViewKyThi_ValidateRow(object sender, DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs e)
        {
            ...
            if (obj != null)
            {
                obj.AddValidationRuleHandler(RuleCheckDuplicate, "MaKyThi");
                if (obj.IsValid)
                    e.Valid = true;
                else
                {
        ...
        {
            KyThi obj = target as KyThi;
            if (obj != null)
            {
                if (((TList<KyThi>)bindingSourceKyThi.DataSource).FindAll(p => p.MaKyThi == obj.MaKyThi).Count > 1)
                {
                    e.Description = string.Format("Mã {0} hiện tại đã có.", obj.MaKyThi);
                    return false;
                }
            }
            return true;
