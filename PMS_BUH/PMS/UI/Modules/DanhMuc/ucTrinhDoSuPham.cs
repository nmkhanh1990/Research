...
{
    public partial class ucTrinhDoSuPham : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public ucTrinhDoSuPham()
        {
            InitializeComponent();
        }

        private void ucTrinhDoSuPham_Load(object sender, EventArgs e)
        {
            ...
            AppGridView.ShowField(gridViewKyThi, new string[] { "MaQuanLy", "TenTrinhDoSuPham", "GhiChu", "NgayCapNhat", "NguoiCapNhat" },
                new string[] { "Mã trình độ sư phạm", "Tên trình độ sư phạm", "Ghi chú", "Ngày cập nhật", "Người cập nhật" },
        ...
        void InitData()
        {
            bindingSourceKyThi.DataSource = DataServices.TrinhDoSuPham.GetAll();
        }
        ...
        private void btnLuu_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            TList<TrinhDoSuPham> list = bindingSourceKyThi.DataSource as TList<TrinhDoSuPham>;
            if (list != null)
            {
                if (XtraMessageBox.Show("Bạn có muốn lưu các thay đổi không ?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    try
                    {
                        if (list.IsValid)
                        {
                            ...
                            DataServices.TrinhDoSuPham.Save(list);
        ...
        private void gridViewKyThi_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            if (col.FieldName == "MaQuanLy" || col.FieldName == "TenTrinhDoSuPham" || col.FieldName == "GhiChu")
            {
        ...
        {
            TrinhDoSuPham obj = e.Row as TrinhDoSuPham;
            if (obj != null)
            {
        ...
        {
            TrinhDoSuPham obj = target as TrinhDoSuPham;
            if (obj != null)
            {
                if (((TList<TrinhDoSuPham>)bindingSourceKyThi.DataSource).FindAll(p => p.MaQuanLy == obj.MaQuanLy).Count > 1)
                {
