...
{
    public partial class ucThanhToanGiangDayLyThuyetGiangVienCoHuuCTIM : DevExpress.XtraEditors.XtraUserControl
    {
        #region Variable
        ...
        public ucThanhToanGiangDayLyThuyetGiangVienCoHuuCTIM()
        {
            InitializeComponent();
        ...
        private void ucThanhToanGiangDayLyThuyetGiangVienCoHuuCTIM_Load(object sender, EventArgs e)
        {
            ...
            AppGridView.ShowField(gridViewThongKe, new string[] { "Chon", "XacNhanChiTra", "MaQuanLy", "HoTen", "ChucDanh", "MaMonHoc", "TenMonHoc", "MaLopHocPhan", "MaLop", "SiSo", "TietThucDay", "HeSoLopDong", "HeSoThamNien", "HeSoChucDanhChuyenMon", "TietQuyDoi", "DonGia", "TongTien", "Thue", "ThucLanh", "NgayCapNhat", "NguoiCapNhat" }
                    , new string[] { "Chọn in", "Khoá thanh toán", "Mã giảng viên", "Họ tên", "Chức danh", "Mã môn học", "Tên môn học", "Mã lớp học phần", "Mã lớp", "Sĩ số", "Số tiết", "Hệ số lớp đông", "Hệ số thâm niên", "Hệ số chức danh", "Số tiết thanh toán", "Đơn giá", "Số tiền thanh toán", "Thuế TNCN", "Thực lãnh", "ngày cập nhật", "Người cập nhật" }
                    , new int[] { 60, 60, 90, 140, 140, 90, 150, 110, 100, 60, 60, 110, 110, 110, 110, 100, 120, 100, 120, 80, 80 });
            AppGridView.AlignHeader(gridViewThongKe, new string[] { "Chon", "XacNhanChiTra", "MaQuanLy", "HoTen", "ChucDanh", "MaMonHoc", "TenMonHoc", "MaLopHocPhan", "MaLop", "SiSo", "TietThucDay", "HeSoLopDong", "HeSoThamNien", "HeSoChucDanhChuyenMon", "TietQuyDoi", "DonGia", "TongTien", "Thue", "ThucLanh", "NgayCapNhat", "NguoiCapNhat" }, DevExpress.Utils.HorzAlignment.Center);
            AppGridView.ReadOnlyColumn(gridViewThongKe, new string[] { "MaQuanLy", "HoTen", "ChucDanh", "MaMonHoc", "TenMonHoc", "MaLopHocPhan", "MaLop", "SiSo", "TietThucDay", "HeSoLopDong", "HeSoThamNien", "HeSoChucDanhChuyenMon", "TietQuyDoi", "DonGia", "TongTien", "Thue", "ThucLanh", "NgayCapNhat", "NguoiCapNhat" });
            ...
            #region Loại học phần


            AppGridLookupEdit.InitGridLookUp(cboLoaiHocPhan, new string[] { "TenLoaiHocPhan" }, new string[] { "Loại học phần" }, new int[] { 220 }, "VietTat", "TenLoaiHocPhan", 300, 200);
              
            cboLoaiHocPhan.EditValue = "LT";
            ...
            if (_maTruong == "CTIM")
            {
                ...
                if (v.Count > 0)
                {
                    _vListKhoaBoMon = v; //_vListKhoaBoMon.FindAll(p => p.MaKhoa == groupname) as VList<ViewKhoa>;
                }
            }
        ...
        void InitData()
        {
            ...
            if (cboNamHoc.EditValue != null && cboHocKy.EditValue != null && cboLanChot.EditValue != null && cboBacDaoTao.EditValue != null && cboKhoaHoc.EditValue != null)
            {
                ...
                IDataReader reader = DataServices.KetQuaThanhToanThuLao.ThongKeTienGiangCtim(cboNamHoc.EditValue.ToString(), cboHocKy.EditValue.ToString(), cboKhoaDonVi.EditValue.ToString(), cboBacDaoTao.EditValue.ToString(), cboKhoaHoc.EditValue.ToString(), "1", cboLoaiHocPhan.EditValue.ToString(), int.Parse(cboLanChot.EditValue.ToString())); //Cơ hữu mã là 1
                dt.Load(reader);
        ...
        {
            Cursor.Current = Cursors.WaitCursor;
            if (cboNamHoc.EditValue != null && cboHocKy.EditValue != null && cboLanChot.EditValue != null && cboBacDaoTao.EditValue != null && cboKhoaHoc.EditValue != null && cboLoaiHocPhan.EditValue != null)
            {
                DataTable dt = new DataTable();
                IDataReader reader = DataServices.KetQuaThanhToanThuLao.ThongKeTienGiangCtim(cboNamHoc.EditValue.ToString(), cboHocKy.EditValue.ToString(), cboKhoaDonVi.EditValue.ToString(), cboBacDaoTao.EditValue.ToString(), cboKhoaHoc.EditValue.ToString(), "1", cboLoaiHocPhan.EditValue.ToString(), int.Parse(cboLanChot.EditValue.ToString())); //Cơ hữu mã là 1
                dt.Load(reader);
        ...
        private void btnIn_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (vListBaoCao != null && vListBaoCao.Rows.Count > 0)
            {
                ...
                {
                    if(cboLoaiHocPhan.EditValue == null)
                    {
                        Common.XuLyGiaoDien.ThongBao("Chọn loại học phần (LT;TH) rồi lọc lại.", MessageBoxIcon.Hand, true);
                    }
                    else if (cboLoaiHocPhan.EditValue.ToString().Contains("LT"))
                    {
                        frm.InThongKeThanhToanTienGiangLyThuyetGiangVienCoHuu(PMS.Common.Globals._cauhinh.TenTruong, cboNamHoc.EditValue.ToString(), cboHocKy.Text, cboKhoaDonVi.Text
                            , PMS.Common.Globals._cauhinh.PhongDaoTao, PMS.Common.Globals._cauhinh.PhongKeHoachTaiChinh, PMS.Common.Globals._cauhinh.BanGiamHieu, PMS.Common.Globals._cauhinh.NguoiLapbieu
                    ...
                    {
                        frm.InThongKeThanhToanTienGiangThucHanhGiangVienCoHuu(PMS.Common.Globals._cauhinh.TenTruong, cboNamHoc.EditValue.ToString(), cboHocKy.Text, cboKhoaDonVi.Text
                            , PMS.Common.Globals._cauhinh.PhongDaoTao, PMS.Common.Globals._cauhinh.PhongKeHoachTaiChinh, PMS.Common.Globals._cauhinh.BanGiamHieu, PMS.Common.Globals._cauhinh.NguoiLapbieu
                            , cboKhoaHoc.Text, cboBacDaoTao.Text, PMS.Common.Globals._cauhinh.ChucVuBanGiamHieu, PMS.Common.Globals._cauhinh.ChucVuKeToan, PMS.Common.Globals._cauhinh.ChucVuDaoTao
                            , _ngayIn, vListBaoCao);
                    }
        ...
        private void gridViewThongKe_CellValueChanging(object sender, CellValueChangedEventArgs e)
        {
            try
            {
