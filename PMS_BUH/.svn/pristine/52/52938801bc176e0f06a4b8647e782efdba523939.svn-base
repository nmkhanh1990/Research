...
{
    public partial class ucDonGia_Kcq : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        void InitGridBUH()
        {
            AppGridView.InitGridView(gridViewDonGia, true, true, DevExpress.XtraGrid.Views.Grid.GridMultiSelectMode.RowSelect, false, true, "Nhấn vào đây để thêm dòng mới");
            AppGridView.ShowEditor(gridViewDonGia, DevExpress.XtraGrid.Views.Grid.NewItemRowPosition.Top);
            AppGridView.ShowField(gridViewDonGia, new string[] { "MaQuanLy", "BacDaoTao", "NgonNguGiangDay", "DonGia", "DonGiaClc", "NgayCapNhat", "NguoiCapNhat" }
                        , new string[] { "Mã quản lý", "Bậc đào tạo", "Ngôn ngữ giảng dạy", "Đơn giá", "Đơn giá CLC", "NgayCapNhat", "NguoiCapNhat" }
                        , new int[] { 100, 110, 120, 100, 120, 99, 99 });
            AppGridView.AlignHeader(gridViewDonGia, new string[] { "MaQuanLy", "BacDaoTao", "NgonNguGiangDay", "DonGia", "DonGiaClc", "NgayCapNhat", "NguoiCapNhat" }, DevExpress.Utils.HorzAlignment.Center);
            ...
            try
            {
                ...
                if (_maTruong == "USSH")
                {
                    if (cboHinhThucDaoTao.EditValue != null)
                        bindingSourceDonGia.DataSource = DataServices.KcqDonGia.GetByHinhThucDaoTao(int.Parse(cboHinhThucDaoTao.EditValue.ToString()));
                }
                else
                    bindingSourceDonGia.DataSource = DataServices.KcqDonGia.GetAll();
            }
            catch 
            {  }
        }
        ...
        private void btnLuu_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            TList<KcqDonGia> list = bindingSourceDonGia.DataSource as TList<KcqDonGia>;
            if (list != null)
            {
                foreach (KcqDonGia d in list)
                ...
                if (XtraMessageBox.Show("Bạn có muốn lưu các thay đổi không?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        if (list.IsValid)
                        {
                            ...
                            DataServices.KcqDonGia.Save(list);
        ...
        {
            KcqDonGia obj = target as KcqDonGia;
            if (obj != null)
            {
                if (((TList<KcqDonGia>)bindingSourceDonGia.DataSource).FindAll(p => p.MaQuanLy == obj.MaQuanLy).Count > 1)
                {
                ...
                if (_maTruong == "BUH")
                {
                    if (((TList<KcqDonGia>)bindingSourceDonGia.DataSource).FindAll(p => p.BacDaoTao == obj.BacDaoTao & p.NgonNguGiangDay == obj.NgonNguGiangDay).Count > 1)
                    {
                    ...
                    if (((TList<KcqDonGia>)bindingSourceDonGia.DataSource).FindAll(p => p.MaLoaiGiangVien == obj.MaLoaiGiangVien & p.MaHocHam == obj.MaHocHam & p.MaHocVi == obj.MaHocVi && p.BacDaoTao == obj.BacDaoTao).Count > 1)
                    {
                        e.Description = "Đơn giá hiện tại đã có.";
                        return false;
                    }
        ...
        {
            KcqDonGia obj = e.Row as KcqDonGia;
            if (obj != null)
            {
