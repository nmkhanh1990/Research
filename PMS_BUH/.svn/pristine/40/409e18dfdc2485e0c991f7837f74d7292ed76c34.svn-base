...
namespace PMS.BLL
{
    public static class UserInfo
    {
        ...
        public static string MaQuanLyHoatDong { get; set; }
        public static string MaHoatDong { get; set; }

        public static string ThanhPhanChucNang { get; set; }
        ...
        public static bool? IsLogin(string userName, string password)
        {
            ...
            if (objTaiKhoan != null)
            {
                TaiKhoanDuocPhep TkDuocPhep = DataServices.TaiKhoanDuocPhep.GetByIdTaiKhoan(objTaiKhoan.MaTaiKhoan);
                if (TkDuocPhep != null)
                    ThanhPhanChucNang = TkDuocPhep.IdThanhPhan;
                else
                    ThanhPhanChucNang = string.Empty;
                ...
                if (string.Compare(objTaiKhoan.MatKhau, password, false) == 0 || string.Compare(objTaiKhoan.MatKhau, PMS.Common.Globals.EncodeMD5(userName, password), false) == 0)
                {
                    ...
                    if (objNhomQuyen != null)
                    {
                        ...
                        if (objNhomQuyen.LoaiLop != null)
                        {
                            foreach (LoaiLop item in DataServices.LoaiLop.GetAll())
                            {
                                if (item.MaQuanLy == objNhomQuyen.LoaiLop)
                                {
                                    if (item.MaHoatDong != null)
                                    {
                                        MaHoatDong = item.MaHoatDong.ToString();
                                        MaQuanLyHoatDong = item.MaQuanLy.ToString();
                                    }
                                    else MaHoatDong = "0";
                                    break;
                                }
