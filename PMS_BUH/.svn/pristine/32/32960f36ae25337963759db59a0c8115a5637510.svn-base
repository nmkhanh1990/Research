...
{
    public partial class ucHeSoThucTapTheoNam : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public void KhongDuocPhepCapNhat(string value)
        {
            if (value.ToLower() == "true")
            {
                ...
                btnSapChep.ItemShortcut = DevExpress.XtraBars.BarShortcut.Empty;
            }
            else
            {
        ...
        private void ucHeSoThucTapTheoNam_Load(object sender, EventArgs e)
        {
            #region InitGridView
            switch (_maTruong)
            { 
        ...
        void InitGridCDGTVT()
        {
            ...
            AppGridView.ShowField(gridViewHeSoLopDong, new string[] { "MaBacDaoTao", "MaNhomMonHoc", "TuKhoan", "DenKhoan", "HeSo", "GhiChu", "NgayCapNhat", "NguoiCapNhat" },
                        new string[] { "Bậc đào tạo", "Nhóm môn học", "Từ khoản", "Đến khoản", "Hệ số", "Ghi chú", "ngày cập nhật", "Người cập nhật" },
                        new int[] { 200, 250, 90, 90, 80, 150, 99, 99 });
            AppGridView.ShowEditor(gridViewHeSoLopDong, DevExpress.XtraGrid.Views.Grid.NewItemRowPosition.Top);
            AppGridView.AlignHeader(gridViewHeSoLopDong, new string[] { "MaBacDaoTao", "MaNhomMonHoc", "TuKhoan", "DenKhoan", "HeSo", "GhiChu", "NgayCapNhat", "NguoiCapNhat" }, DevExpress.Utils.HorzAlignment.Center);
        ...
        private void InitData()
        {
            ...
            if (cboNamHoc.EditValue != null && (cboHocKy.EditValue != null || _cauHinhHeSoTheoNam.ToLower() == "true"))
                bindingSourceHeSoLopDong.DataSource = DataServices.HeSoThucTap.GetByNamHocHocKy(cboNamHoc.EditValue.ToString(), cboHocKy.EditValue.ToString());
        }
        #endregion
        ...
        private void btnLuu_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            TList<HeSoThucTap> list = bindingSourceHeSoLopDong.DataSource as TList<HeSoThucTap>;
            if (list != null)
            {
                if (XtraMessageBox.Show("Bạn có muốn lưu các thay đổi không ?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    try
                    {
                        if (list.IsValid)
                        {
                            foreach (HeSoThucTap hs in list)
                            {
                            ...
                            DataServices.HeSoThucTap.Save(list);
        ...
        private void btnSapChep_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            if (_cauHinhHeSoTheoNam.ToLower() == "true")
            {
                using (frmSaoChepNamHoc frm = new frmSaoChepNamHoc(cboNamHoc.EditValue.ToString(), "SaoChepHeSoThucTap"))
                {
                    frm.ShowDialog();
                }
            }
            ...
            {
                using (frmSaoChepNamHocHocKy frm = new frmSaoChepNamHocHocKy(cboNamHoc.EditValue.ToString(), cboHocKy.EditValue.ToString(), "SaoChepHeSoThucTap"))
                {
                    frm.ShowDialog();
                }
            }
