...
{
    public partial class ucHeSoDiaDiem_SDH : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public ucHeSoDiaDiem_SDH()
        {
            InitializeComponent();
            _maTruong = _cauHinhChung.Find(p => p.TenCauHinh == "Mã trường").GiaTri;
        }

        private void ucHeSoDiaDiem_SDH_Load(object sender, EventArgs e)
        {
            ...
            #region Init Data
            bindingSourceHeSoDiaDiem.DataSource = DataServices.SdhHeSoDiaDiem.GetAll();
            #endregion
        }
        ...
        {
            SdhHeSoDiaDiem obj = target as SdhHeSoDiaDiem;
            if (obj != null)
            {
                if (((TList<SdhHeSoDiaDiem>)bindingSourceHeSoDiaDiem.DataSource).FindAll(p => p.MaQuanLy == obj.MaQuanLy).Count > 1)
                {
        ...
        {
            SdhHeSoDiaDiem obj = new SdhHeSoDiaDiem();
            if (obj != null)
            {
        ...
        private void btnLuu_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            TList<SdhHeSoDiaDiem> list = bindingSourceHeSoDiaDiem.DataSource as TList<SdhHeSoDiaDiem>;
            if (list != null)
            {
                if (XtraMessageBox.Show("Bạn có muốn lưu các thay đổi không ?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    try
                    {
                        if (list.IsValid)
                        {
                            ...
                            DataServices.SdhHeSoDiaDiem.Save(list);
