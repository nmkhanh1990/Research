...
{	
	...
	{		
		...
		public static TList<LoaiKhoiLuong> Fill(IDataReader reader, TList<LoaiKhoiLuong> rows, int start, int pageLength)
		{
			...
			for (int i = 0; i < pageLength; i++)
			{
					...
					))
				{
					...
					c.TinhTrang = (reader[LoaiKhoiLuongColumn.TinhTrang.ToString()] == DBNull.Value) ? null : (System.Boolean?)reader[(LoaiKhoiLuongColumn.TinhTrang.ToString())];
		...
		public static void RefreshEntity(IDataReader reader, PMS.Entities.LoaiKhoiLuong entity)
		{
			...
			entity.TinhTrang = (reader[LoaiKhoiLuongColumn.TinhTrang.ToString()] == DBNull.Value) ? null : (System.Boolean?)reader[(LoaiKhoiLuongColumn.TinhTrang.ToString())];
