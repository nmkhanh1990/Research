...
{
    public partial class frmChotTongKetKhoiLuongGiangDayQuyCheMoi : DevExpress.XtraEditors.XtraForm
    {
        ...
        #region Event Form
        public frmChotTongKetKhoiLuongGiangDayQuyCheMoi()
        {
            InitializeComponent();
        ...
        private void frmChotTongKetKhoiLuongGiangDayQuyCheMoi_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    AppGridView.InitGridView(gridViewTongKet, true, true, DevExpress.XtraGrid.Views.Grid.GridMultiSelectMode.RowSelect, false, true);
                    AppGridView.ShowField(gridViewTongKet, new string[] { "MaQuanLy", "HoTen", "MaHocHam", "MaHocVi", "MaLoaiGiangVien", "MaMonHoc", "TenMonHoc", "Loai", "Nhom", "MaLop", "SiSo", "SoTiet", "HeSoLopDong", "SoGioThucGiangTrenLop", "SoGioChuanTinhThem", "HeSoHocKy", "DonGia", "TongThanhTien", "TietQuyDoi", "MaDonVi" }
                            , new string[] { "Mã giảng viên", "Họ và tên", "Học hàm", "Học vị", "Loại giảng viên", "Mã môn học", "Tên môn học", "Loại", "Nhóm", "Mã lớp", "Sĩ số", "Số tiết", "Hệ số lớp đông", "Giờ thực giảng trên lớp", "Giờ chuẩn tính thêm", "Hệ số học kỳ", "Đơn giá", "Thành tiền", "Tiết quy đổi", "Đơn vị" }
                            , new int[] { 90, 140, 80, 80, 80, 80, 150, 60, 70, 80, 70, 70, 100, 110, 110, 90, 80, 90, 90, 90 });
                    AppGridView.AlignHeader(gridViewTongKet, new string[] { "MaQuanLy", "HoTen", "MaMonHoc", "TenMonHoc", "Loai", "Nhom", "MaLop", "SiSo", "SoTiet", "HeSoLopDong", "SoGioThucGiangTrenLop", "SoGioChuanTinhThem", "HeSoHocKy", "DonGia", "TongThanhTien", "TietQuyDoi" }, DevExpress.Utils.HorzAlignment.Center);
                    AppGridView.SummaryField(gridViewTongKet, "MaQuanLy", "{0}", DevExpress.Data.SummaryItemType.Count);
                    AppGridView.ReadOnlyColumn(gridViewTongKet);
                    AppGridView.FormatDataField(gridViewTongKet, "SiSo", DevExpress.Utils.FormatType.Custom, "n0");
                    AppGridView.FormatDataField(gridViewTongKet, "SoTiet", DevExpress.Utils.FormatType.Custom, "n2");
        ...
        void InitData()
        {
            ...
            if (cboNamHoc.EditValue != null && cboHocKy.EditValue != null)
            {
                if (_maTruong == "USSH" || _maTruong == "ACT" || _maTruong == "UEL")
                    bindingSourceTongKet.DataSource = DataServices.ViewLietKeKhoiLuongGiangDayChiTietUssh.GetByNamHocHocKy(cboNamHoc.EditValue.ToString(), cboHocKy.EditValue.ToString(),string.Empty,string.Empty);
                else
                    bindingSourceTongKet.DataSource = DataServices.ViewLietKeKhoiLuongGiangDayChiTiet2.GetByNamHocHocKy(cboNamHoc.EditValue.ToString(), cboHocKy.EditValue.ToString());
        ...
        private void btnChotThanhToan_Click(object sender, EventArgs e)
        {
            ...
            if (XtraMessageBox.Show(string.Format("Chốt khối lượng giảng dạy năm học {0} - {1} lần thứ {2}?", cboNamHoc.EditValue.ToString(), cboHocKy.EditValue.ToString(), lanchot), "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                try
                {
                    DataServices.KetQuaThanhToanThuLao.ChotThanhToan2(cboNamHoc.EditValue.ToString(), cboHocKy.EditValue.ToString(), lanchot, ref kq);
                    DataServices.KetQuaThanhToanThuLao.GetSoLanChot(cboNamHoc.EditValue.ToString(), cboHocKy.EditValue.ToString(), ref lanchot);
                    txtSoLanChot.Text = lanchot.ToString();
                    if (kq == 0)
