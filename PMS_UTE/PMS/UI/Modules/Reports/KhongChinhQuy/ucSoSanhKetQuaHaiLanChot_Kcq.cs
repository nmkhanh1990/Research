...
{
    public partial class ucSoSanhKetQuaHaiLanChot_Kcq : DevExpress.XtraEditors.XtraUserControl
    {
        TList<CauHinhChung> _cauHinhChung = DataServices.CauHinhChung.GetAll();
        int lanchot = 0;
        string _maTruong;
        public ucSoSanhKetQuaHaiLanChot_Kcq()
        {
            InitializeComponent();
        ...
        private void ucSoSanhKetQuaHaiLanChot_Load(object sender, EventArgs e)
        {
            ...
            {
                AppGridView.InitGridView(gridViewSoSanh, true, true, DevExpress.XtraGrid.Views.Grid.GridMultiSelectMode.RowSelect, true, true);
                AppGridView.ShowField(gridViewSoSanh, new string[] { "TenDonVi", "MaQuanLy", "HoTen", "MaMonHoc1", "TenMonHoc1", "MaMonHoc2", "TenMonHoc2", "LoaiHocPhan1", "LoaiHocPhan2", "Nhom1", "Nhom2", "MaLop1", "MaLop2", "SiSo1", "SiSo2", "LopClc1", "LopClc2", "TietThucDay1", "TietThucDay2", "HeSoLopDong1", "HeSoLopDong2", "HeSoHocKy1", "HeSoHocKy2", "TietQuyDoi1", "TietQuyDoi2", "DonGia1", "DonGia2", "ThanhTien1", "ThanhTien2" }
                        , new string[] { "Tên khoa", "Mã giảng viên", "Họ tên", "Mã môn học A", "Tên môn học A", "Mã môn học B", "Tên môn học B", "Loại học phần A", "Loại học phần B", "Nhóm A", "Nhóm B", "Mã lớp A", "Mã lớp B", "Sĩ số A", "Sĩ số B", "Lớp CLC A", "Lớp CLC B", "Tiết thực dạy A", "Tiết thực dạy B", "Hệ số lớp đông A", "Hệ số lớp đông B", "Hệ số học kỳ A", "Hệ số học kỳ B", "Tiết quy đổi A", "Tiết quy đổi B", "Đơn giá A", "Đơn giá B", "Thành tiền A", "Thành tiền B" }
                        , new int[] { 120, 90, 140, 80, 150, 80, 150, 120, 120, 60, 60, 70, 70, 70, 70, 90, 90, 120, 120, 110, 110, 100, 100, 100, 100, 90, 90, 100, 100 });
        ...
        void InitData()
        {
            ...
            if (cboNamHoc.EditValue != null && cboHocKy.EditValue != null && cboLanChotA.EditValue != null && cboLanChotB.EditValue != null)
            {
                ...
                IDataReader reader = DataServices.KcqKetQuaThanhToanThuLao.SoSanhHaiLanChot(cboNamHoc.EditValue.ToString(), cboHocKy.EditValue.ToString(), int.Parse(cboLanChotA.EditValue.ToString()), int.Parse(cboLanChotB.EditValue.ToString()));
                dt.Load(reader);
        ...
        void InitLanChot()
        {
            if (cboNamHoc.EditValue != null && cboHocKy.EditValue != null)
            {
                ...
                cboLanChotA.DataBindings.Add("EditValue", bindingSourceLanChotA, "LanChot", true, DataSourceUpdateMode.Never);
                cboLanChotB.DataBindings.Add("EditValue", bindingSourceLanChotB, "LanChot", true, DataSourceUpdateMode.Never);
            }
        }
        #endregion
