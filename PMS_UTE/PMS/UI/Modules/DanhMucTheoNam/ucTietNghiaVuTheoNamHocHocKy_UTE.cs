...
namespace PMS.UI.Modules.DanhMuc
{
    public partial class ucTietNghiaVuTheoNamHocHocKy_UTE : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private void ucTietNghiaVuTheoNamHocHocKy_UTE_Load(object sender, EventArgs e)
        {
            ...
            AppGridView.ShowField(gridViewTietNghiaVu, new string[] { "MaQuanLy", "SoHieuCongChuc", "HoTen", "TenLoaiGiangVien", "TenHocHamHrm", "TenHocHam"
                    , "TenHocViHrm", "TenHocVi", "TenBoMon", "TenKhoa", "TenChucVuHrm", "TenChucVu", "TenHinhThucDaoTao", "DinhMucToiThieu", "DinhMucCaNam", "DinhMucHocKy", "TenTinhTrangHrm", "TenTinhTrang"
                    , "TinhTrangCongViec", "TuNgay", "DenNgay", "TietGioiHan", "MienGiamPhanTramNghiaVu", "DinhMucSauGiamTru", "TietNghiaVuBoSung", "TietNghiaVuThucHien", "GhiChu2"
                    , "MaGiangVien", "GhiChu", "MaHocHam", "MaHocVi", "Huong85PhanTram", "TenChucDanh", "TenLoaiNhanSu", "TenNgachLuong", "TenLoaiNhanVien", "TienCanTren", "BiChan", "CoGiangDay" }
                , new string[] { "Mã giảng viên", "Số hiệu công chức", "Họ và tên", "Loại giảng viên", "Học hàm HRM", "Học hàm"
                    , "Học vị HRM", "Học vị", "Bộ môn", "Khoa", "Chức vụ HRM", "Chức vụ", "Hình thức đăng ký", "Phần trăm chức vụ", "Định mức cả năm", "Định mức học kỳ", "Tình trạng HRM", "Tình trạng"
                    , "Tình trạng công việc", "Từ ngày", "Đến ngày", "Số tiết giới hạn", "Phần trăm miễn giảm nghĩa vụ", "Tiết nghĩa vụ sau giảm trừ", "Tiết nghĩa vụ bổ sung", "Tiết nghĩa vụ thực hiện", "Ghi chú"
                    , "Mã giảng viên", "Cột để tô màu", "MaHocHam", "MaHocVi", "Hưởng 85% lương", "Chức danh HRM", "Loại nhân sự HRM", "Ngạch lương HRM", "Loại nhân sự PMS", "Tiền tối đa", "Bị chặn", "Có giảng dạy" }
                , new int[] { 90, 80, 140, 90, 90, 90, 90, 90, 120, 120, 90, 90, 120, 80, 80, 80, 80, 90, 90, 90, 90, 80, 80, 90, 90, 90, 100, 99, 99, 99, 99, 90, 90, 90, 90, 90, 90, 50, 60 });
            ...
            AppGridView.ReadOnlyColumn(gridViewTietNghiaVu, new string[] { "MaQuanLy", "SoHieuCongChuc", "HoTen", "TenLoaiGiangVien", "TenHocHamHrm", "TenHocHam", "TenHocViHrm", "TenHocVi", "TenBoMon", "TenKhoa", "TenChucVuHrm", "TenChucVu", "TenHinhThucDaoTao", "DinhMucToiThieu", "DinhMucCaNam", "DinhMucHocKy", "DinhMucSauGiamTru", "TenTinhTrangHrm", "TenTinhTrang", "TinhTrangCongViec", "TuNgay", "DenNgay", "TietNghiaVuThucHien", "MaGiangVien", "Huong85PhanTram", "TenChucDanh", "TenLoaiNhanSu", "TenNgachLuong", "TenLoaiNhanVien", "TienCanTren", "CoGiangDay" });
            AppGridView.HideField(gridViewTietNghiaVu, new string[] { "MaGiangVien", "BiChan", "GhiChu", "MaHocHam", "MaHocVi" });
                ...
                if (list != null)
                {
                    ...
                    { 
                        ...
                        foreach (DataRow kl in list.Rows)
                        {
                            ...
                            //{
                            xmlData += String.Format("<Input M=\"{0}\" Hh=\"{1}\" Hv=\"{2}\" T=\"{3}\" G=\"{4}\" G2=\"{5}\" P=\"{6}\" B=\"{7}\"/>"
                                        , kl["MaGiangVien"]
                                        , PMS.Common.Globals.ReplaceDotEnviroment(kl["MaHocHam"], "int", true)
                                        , PMS.Common.Globals.ReplaceDotEnviroment(kl["MaHocVi"], "int", true)
                                        , PMS.Common.Globals.ReplaceDotEnviroment(kl["TietNghiaVuThucHien"], "decimal", true)
                                        , PMS.Common.Globals.ReplaceDotEnviroment(kl["GhiChu"], "bool", true)
                                        //, PMS.Common.Globals.IsNull(kl["TietGioiHan"], "decimal")
                                        , kl["GhiChu2"]
                                        , PMS.Common.Globals.ReplaceDotEnviroment(kl["MienGiamPhanTramNghiaVu"], "decimal", true)
                                        , PMS.Common.Globals.ReplaceDotEnviroment(kl["TietNghiaVuBoSung"], "decimal", true));
                                            //, kl["BiChan"] );
                            //}
                        }
        ...
        private void gridViewTietNghiaVu_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            try
            {
                ...
                if (col.FieldName == "TietNghiaVuBoSung")
                {
                    decimal _tietNghiaVuThucHien = decimal.Parse(PMS.Common.Globals.IsNull(v["DinhMucSauGiamTru"], "decimal").ToString())
                ...
                else if (col.FieldName == "MienGiamPhanTramNghiaVu")
                {
                ...
                {
                    //gridViewTietNghiaVu.SetRowCellValue(pos, "GhiChu2", "Thay đổi chặn vào " + DateTime.Now.Day + "/" + DateTime.Now.Month + "/" + DateTime.Now.Year);
                }
            }
            catch
            { }
