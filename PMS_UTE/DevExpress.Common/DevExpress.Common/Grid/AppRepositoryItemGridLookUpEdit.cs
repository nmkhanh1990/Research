...
namespace DevExpress.Common.Grid
{
    public static class AppRepositoryItemGridLookUpEdit
    {
        ...
        /// <param name="autoPopup"></param>
        /// <param name="textEditStyle"></param>
        public static void InitRepositoryItemGridLookUp(RepositoryItemGridLookUpEdit grid, string[] showFields, string[] showFieldNames, string valueMember, string displayMember, int width, int height)
        {
            //Show filter
            grid.View.OptionsView.ShowAutoFilterRow = true;
            grid.TextEditStyle = TextEditStyles.Standard;
            grid.ImmediatePopup = true;
            grid.PopupFilterMode = PopupFilterMode.Contains;

            grid.ValueMember = valueMember;
            grid.DisplayMember = displayMember;
            grid.NullText = string.Empty;

            //grid.BestFitMode = BestFitMode.BestFit;
            for (int i = 0; i < grid.View.Columns.Count; i++)
                grid.View.Columns[i].Visible = false;

            ShowField(grid, showFields, showFieldNames);
        ...
        /// <param name="autoPopup"></param>
        /// <param name="textEditStyle"></param>
        public static void InitRepositoryItemGridLookUp(RepositoryItemGridLookUpEdit grid, string[] showFields, string[] showFieldNames, int[] columnWidth, string valueMember, string displayMember, int width, int height)
        {
            //Show filter
            grid.View.OptionsView.ShowAutoFilterRow = true;
            grid.TextEditStyle = TextEditStyles.Standard;
            ...
            ShowField(grid, showFields, showFieldNames, columnWidth);
            grid.PopupFormSize = new Size(width, height);
        }
